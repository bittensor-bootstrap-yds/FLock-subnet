...
def is_up_to_date_with_main():
    ...
    try:
        ...
        if is_up_to_date:
            bt.logging.info(f"Current branch is up to date with origin/main ({main_commit[:8]})")
        else:
            bt.logging.error(f"Current branch does NOT contain the latest commit from origin/main ({main_commit[:8]})")
...
def check_latest_code():
    ...
    if current_branch is None:
        raise RuntimeError("Failed to determine current git branch. Is this a git repository?")
